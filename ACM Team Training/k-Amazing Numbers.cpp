...
int main(void) {
	...
	while (t--)
	{
		...
		for0n(n) {
			scanf("%lld", a + i);
			dis[a[i]] = max(dis[a[i]], i - last[a[i]] + 1);
			last[a[i]] = i;
		}
		for0(n + 10) {
			ans[i] = 0x3f3f3f3f;
		}
		//sort(dis, dis + n, [](ll a, ll b)->bool {return a > b; });
		for0(n) {
			dis[a[i]] = max(dis[a[i]], n - last[a[i]] + 1);
		}
		for (ll i = 0; i < n; i++) {
			ans[dis[a[i]]] = min(ans[dis[a[i]]], a[i]);
		}
		for (ll i = 0; i < n; i++) {
			ans[dis[a[i]]] = min(ans[dis[a[i]]], a[i]);
		}
		...
		for1(n) {
			printf("%lld ", ans[i + 1] == 0x3f3f3f3f ? -1 : ans[i + 1]);
		}
		printf("\n");
	}
	return 0;
